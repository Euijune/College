...
//       logical predecessor and the predecessor's node is deleted.
{
  ...
  {
    tempPtr = PtrToSuccessor_recur(tree->right);
    data = tempPtr->info;
    tree->info = data;
    delete tempPtr;    
  }
...
void TreeType::Ancestors(ItemType value)
{
    ...
    // value를 가진 노드를 찾거나 트리 끝에 도달할 때까지
    // leafNode->info == value일 경우를 고려하여 while조건문을 설정함
    while (!found) 
    {
        if (currentNode->info == value) // value를 가진 노드를 찾으면
            found = true;
        else if (currentNode->left == NULL && currentNode->right == NULL) // 현재 위치가 트리의 끝이라면
            break;
        else {
            path.Enqueue(currentNode->info); // path에 현재 노드의 값을 삽입
            if (currentNode->info > value && currentNode->left != NULL)
                currentNode = currentNode->left;
            else if (currentNode->info < value && currentNode->right != NULL)
                currentNode = currentNode->right;
            else  // 경로가 없는 경우 (Ex : currNode.info < value 인데, right child가 없는 경우)
                break;
        }
    }
...
bool Ancestors_recursive(TreeNode* tree, ItemType item)
{
    ...
    if (found) // 재귀호출 후 value값을 찾았으면
        std::cout << tree->info << " ";
    // value값을 찾지 못한 경우, 아무 출력도 하지 않는다.

...
{
    /*
        문제가 발생한 부분 : 함수의 시작에 return 0 이 존재하므로, list의 info값에 상관없이 0만 return됨.
    */
    if (list != NULL)
...
{
    /*
        문제가 발생한 부분 : 각 리스트 요소의 제곱을 더해야 하는데, 제곱을 하지않고 sum값을 구함.
    */
    int sum = 0;
    while (list != NULL) {
...
{
    /*
        문제가 발생한 부분 : 39의 제곱근은 자연수가 아니다. 
        즉, SumSquares_e * SumSquares_e = 39 인 SumSquares_e의 값은 list의 원소 1,2,3,5 로 만들 수 없다.
    */

...
private:
  NodeType<ItemType>* listData; // 가장 큰 node를 가리킴
  int length;
    ...
    //      logical predecessor.
{
    ...
    location = listData->next; // 현재 listData는 sortedLL의 맨 마지막 원소를 가리키고 있기 때문.
    predLoc = listData;
    found = false;
...
void SortedType<ItemType>::DeleteItem(ItemType item)
{
    ...
    FindItem(listData, item, location, predLoc, found);
    // cout << "test in SortedType::DeleteItem " << predLoc->info << " " << location->info << " " << listData->info << endl;
    if (predLoc == location) // Only one node in list?
        listData = NULL;
    else
    {
...
// Post: Current position has been initialized. 즉, GetNextItem 쓰기 전 이 함수 호출이 필요함.
{
  currentPos = NULL;
}

...
TreeNode* TreeType::PtrToSuccessor_recur(TreeNode*& tree) // 파라메터 타입: (1) TreeNode* or (2) TreeNode*&
{
...
TreeNode* TreeType::PtrToSuccessor_nonrecur(TreeNode*& tree) // 파라메터 타입: (1) TreeNode* or (2) TreeNode*&
{
...
bool Imp_IsBST(TreeNode* tree, ItemType& min, ItemType& max) // min, max: returns the value range of the tree
{
    ...
    if (tree->left != NULL) {
        ...
        if (!isBST || tree->info <= left_min) return false;
    }
    //오른쪽 노드가 NULL이 아니면, 오른쪽 서브트리가 BST인지 체크하고 tree->info와 비교
    if (tree->right != NULL) {
        isBST = Imp_IsBST(tree->right, right_min, right_max);
        if (!isBST || tree->info >= right_max) return false;
    }

    min = (tree->left == NULL) ? tree->info : left_min; 
    max = (tree->right == NULL) ? tree->info : right_max; ; // min, max의 값을 설정
    return true;
}
...
int Imp_LeafCount(TreeNode* tree)
{
    if (tree == NULL) // 노드가 아닐 경우.
        return 0;
    else if (tree->left == NULL && tree->right == NULL) //노드의 좌우가 NULL일 경우 = leaf Node일 경우
        return 1;
    else
...
int Imp_SingleParentCount(TreeNode* tree)
{
    if (tree == NULL)
        return 0;
    else if (tree->left == NULL && tree->right != NULL) // 오른쪽 자식노드만 존재하는 경우
        return 1 + Imp_SingleParentCount(tree->right);
    else if (tree->right == NULL && tree->left != NULL) // 왼쪽 자식노드만 존재하는 경우
